...
const PLAYLIST = [
  { title: "DigBar - 22 Grand", src: "assets/grand22.mp3" },
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function startBannerTypewriter() {
    ...
    function step() {
      if (isDeleting) {
        index--;
        bannerTypingTitle.textContent = SITE_NAME.substring(0, index);
        document.title = index > 0 ? SITE_NAME.substring(0, index) : SITE_NAME;
      } else {
        index++;
        bannerTypingTitle.textContent = SITE_NAME.substring(0, index);
        document.title = SITE_NAME.substring(0, index);
      }
  ...
  function setRandomTrackAndBackground() {
    ...
    const randomBgFile = BACKGROUNDS[Math.floor(Math.random() * BACKGROUNDS.length)];
    if (bgGif) {
