...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initUniqueViewCounter() {
    const VISITED_KEY = 'abbys_unique_visitor_v1';
    const LOCAL_VIEWS_KEY = 'abbys_total_unique_views';
    const hasVisited = localStorage.getItem(VISITED_KEY);

    try {
      let countUrl = 'https://api.counterapi.dev/v1/abbys.site/unique_views';
      ...
      console.log('Counter API fallback mode:', e);
      let currentViews = parseInt(localStorage.getItem(LOCAL_VIEWS_KEY) || '142', 10);
      if (!hasVisited) {
        currentViews += 1;
  ...
  function setRandomTrackAndBackground() {
    if (PLAYLIST.length > 0) {
      let nextTrackIndex = (lastTrackIndex + 1) % PLAYLIST.length;
      lastTrackIndex = nextTrackIndex;
      try {
        localStorage.setItem(LOCAL_TRACK_KEY, nextTrackIndex.toString());
      } catch (e) {}
    ...
    if (BACKGROUNDS.length > 0) {
      let nextBgIndex = (lastBgIndex + 1) % BACKGROUNDS.length;
      lastBgIndex = nextBgIndex;
      try {
        localStorage.setItem(LOCAL_BG_KEY, nextBgIndex.toString());
      } catch (e) {}

      const randomBgFile = BACKGROUNDS[nextBgIndex];
      if (bgGif) {
        bgGif.onerror = () => {
          console.log('Background load error, falling back to bg_static.jpg');
          bgGif.src = './assets/bg_static.jpg';
        };
