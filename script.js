...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function initUniqueViewCounter() {
    const VISITED_KEY = 'abbys_site_visited_v4';
    const LOCAL_VIEWS_KEY = 'abbys_total_unique_views_v4';
    ...
    try {
      const endpoint = hasVisited 
        ? 'https://api.counterapi.dev/v1/abbyssiteofficialcounter/views' 
        : 'https://api.counterapi.dev/v1/abbyssiteofficialcounter/views/up';
    ...
    try {
      const response = await fetch('https://api.codetabs.com/v1/counter?key=abbyssiteofficialcounter');
      const data = await response.json();
      let countVal = typeof data === 'number' ? data : (data && data.count);
